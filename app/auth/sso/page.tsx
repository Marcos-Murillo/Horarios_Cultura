...
import type { HorariosUser } from "@/lib/auth-context"
  ...
  useEffect(() => {
    ...
    fetch("/api/auth/verify-sso", {
      ...
      .then((data) => {
        ...
        login(user)
        router.replace(searchParams.get("redirect") ?? "/adofi")
      })
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-3 bg-gray-50">
        ...
        <a href="/sin-acceso" className="text-gray-600 underline text-sm">Volver</a>
      </div>
    )
  }
